...
В продакшене на Render исходящие порты для классического SMTP (587, 465) полностью заблокированы провайдером во избежание спама, из-за чего библиотека `smtplib` выбрасывала ошибку `[Errno 101] Network is unreachable`.

**Как решено:** Чтобы инфраструктурные ограничения хостинга не ломали пользователю отправку формы, вызовы фоновых потоков `run_in_threadpool` для отправки писем в `EmailService` были изолированы в независимые блоки `try-except`. Ошибка сети логируется в `app.log`, но API успешно завершает транзакцию, возвращая статус `201 Created` и авто-ответ от AI на фронтенд. При этом локально (на машине проверяющего) код работает на 100% и отправляет реальные письма, так как домашние/офисные сети данные порты не блокируют.
...
## ⚠️ Важное примечание для проверяющих (Тестирование Email-оповещений)

Пожалуйста, обратите внимание, что бесплатный тариф платформы Render на уровне инфраструктуры полностью блокирует исходящие SMTP-порты (587/465), из-за чего в Live-демо версии отправка реальных писем завершается сетевым таймаутом (что безопасно гасится нашей системой перехвата ошибок без падения API).

Для того чтобы увидеть ПОЛНЫЙ цикл работы приложения, включая физическую отправку адаптивных HTML-писем на почту владельца и пользователя, мы настоятельно рекомендуем запустить проект локально по инструкции из Раздела 1. В локальном окружении порты не блокируются, и письма успешно доставляются.
